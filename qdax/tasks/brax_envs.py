...
from qdax.tasks import environments
...
def reset_based_scoring_actor_dc_function_brax_envs(
	...
	random_key, subkey = jax.random.split(random_key)
	keys = jax.random.split(subkey, jax.tree.leaves(actors_dc_params)[0].shape[0])
	reset_fn = jax.vmap(play_reset_fn)
	init_states = reset_fn(keys)
...
def create_brax_scoring_fn(
	...
	play_reset_fn: Callable[[RNGKey], EnvState] | None = None,
) -> tuple[
...
def get_aurora_scoring_fn(
	...
	[Genotype, RNGKey], tuple[Fitness, Descriptor | None, ExtraScores, RNGKey]
]:
	...
	def _wrapper(
		params: Params,
		random_key: RNGKey,  # Perform rollouts with each policy
	) -> tuple[Fitness, Descriptor | None, ExtraScores, RNGKey]:
		fitnesses, _, extra_scores, random_key = scoring_fn(params, random_key)
		data = extra_scores["transitions"]
		observation = observation_extractor_fn(data)
