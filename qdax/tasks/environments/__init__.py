...
from qdax.tasks.environments.exploration_wrappers import MazeWrapper, TrapWrapper
from qdax.tasks.environments.humanoidtrap import HumanoidTrap
...
from qdax.tasks.environments.pointmaze import PointMaze
from qdax.tasks.environments.wrappers import CompletedEvalWrapper
...
behavior_descriptor_extractor = {
	...
	"antblock_omni": get_final_xy_position,
	"antblockmany_omni": get_final_xy_position,
	...
	"ant_omni_nobound": get_final_xy_position,
...
def create(
	...
	**kwargs: Any,
	...
	if env_name in _envs.keys():
		...
		elif base_env_name in _qdax_envs:
			env = _qdax_envs[base_env_name](**kwargs)
	else:
	...
	if fixed_init_state:
		...
		if env_name not in _qdax_custom_envs:
			base_env_name = env_name
		# wrap the env
		env = FixedInitialStateWrapper(env, base_env_name=base_env_name)
	if auto_reset:
