...
from qdax.tasks.environments.locomotion_wrappers import COG_NAMES
...
class TrapWrapper(Wrapper):
	...
	def __init__(self, env: Env, env_name: str) -> None:
		if (
			env_name not in ENV_SYSTEM_CONFIG
			or env_name not in COG_NAMES.keys()
			or env_name not in ENV_TRAP_COLLISION
		):
...
class MazeWrapper(Wrapper):
	...
	def __init__(self, env: Env, env_name: str) -> None:
		if (
			env_name not in ENV_SYSTEM_CONFIG
			or env_name not in COG_NAMES.keys()
			or env_name not in ENV_MAZE_COLLISION
		):
