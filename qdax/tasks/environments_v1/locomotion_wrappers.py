...
from qdax.tasks.environments_v1.base_wrappers import QDEnv
...
class QDSystem(System):
	...
	def __init__(
		self, config: config_pb2.Config, resource_paths: Sequence[str] | None = None
	):
...
class XYPositionWrapper(QDEnv):
	...
	def __init__(
		...
		env: Env,
		env_name: str,
		minval: list[float] | None = None,
		maxval: list[float] | None = None,
	):
...
FORWARD_REWARD_NAMES = {
	...
	"antblock": "reward_forward",
	"antblockmany": "reward_forward",
...
class NoForwardRewardWrapper(Wrapper):
	...
	def __init__(self, env: Env, env_name: str) -> None:
		if env_name not in FORWARD_REWARD_NAMES:
			raise NotImplementedError(f"This wrapper does not support {env_name} yet.")
		super().__init__(env)
		self._env_name = env_name
