...
class AURORA:
	...
	def __init__(
		...
		emitter: Emitter,
		metrics_function: Callable[[GridPopulation], Metrics],
		encoder_function: Callable[[Observation, AuroraExtraInfo], Descriptor],
		training_function: Callable[
			[RNGKey, ThresholdPopulation, Params, int], AuroraExtraInfo
		],
	) -> None:
	...
	def train(
		self,
		repertoire: ThresholdPopulation,
		model_params: Params,
		iteration: int,
		random_key: RNGKey,
	) -> tuple[ThresholdPopulation, AuroraExtraInfo]:
	...
	) -> tuple[ThresholdPopulation, jnp.ndarray]:
		# update the l value
	...
	def init(
		...
		max_size: int,
		random_key: RNGKey,
	) -> tuple[ThresholdPopulation, EmitterState | None, AuroraExtraInfo, RNGKey]:
		...
		repertoire = ThresholdPopulation.init(
			genotypes=genotypes,
			fitnesses=fitnesses,
	...
	def update(
		...
		random_key: RNGKey,
		aurora_extra_info: AuroraExtraInfo,
	) -> tuple[GridPopulation, EmitterState | None, Metrics, RNGKey]:
