...
from qdax.core.populations.grid_population import compute_cvt_centroids
from qdax.tasks.brax_envs import (
	reset_based_scoring_actor_dc_function_brax_envs as scoring_actor_dc_function,
)
from qdax.tasks.brax_envs import (
...
def main(config: DictConfig) -> None:
	...
	assert (
		config.batch_size
		== config.algo.ga_batch_size
		+ config.algo.qpg_batch_size
		+ config.algo.ai_batch_size
	)
	...
	def play_step_actor_dc_fn(env_state, actor_dc_params, desc, random_key):
		...
		actions = actor_dc_network.apply(
			actor_dc_params, env_state.obs, desc_prime_normalized
		)
		state_desc = env_state.info["state_descriptor"]
		next_state = env.step(env_state, actions)
	...
	def evaluate_actor(random_key, repertoire, actor_params):
		...
		actors_params = jax.tree.map(
			lambda x: jnp.repeat(
				jnp.expand_dims(x, axis=0), config.num_centroids, axis=0
			),
			actor_params,
		)
	...
	def get_elites(metric):
		split = jnp.cumsum(
			jnp.array([emitter.batch_size for emitter in map_elites._emitter.emitters])
		)
		split = jnp.split(metric, split, axis=-1)[:-1]
		qpg_offspring_added, ai_offspring_added = jnp.split(
			split[0], (config.algo.qpg_batch_size,), axis=-1
		)
		return (
			jnp.sum(split[1], axis=-1),
			jnp.sum(qpg_offspring_added, axis=-1),
			jnp.sum(ai_offspring_added, axis=-1),
		)
	...
	for i in range(num_loops):
		...
		random_key, qd_score_actor, dem_actor = evaluate_actor(
			random_key, repertoire, emitter_state.emitter_states[0].actor_params
		)
		...
		if i == -1:
			(
				...
				current_metrics["qpg_offspring_added"],
				current_metrics["ai_offspring_added"],
			) = get_elites(current_metrics["is_offspring_added"] + is_offspring_added)
			(
				current_metrics["ga_improvement"],
				current_metrics["qpg_improvement"],
				current_metrics["ai_improvement"],
			) = get_elites(current_metrics["improvement"] + improvement)
		else:
			(
				current_metrics["ga_offspring_added"],
				current_metrics["qpg_offspring_added"],
				current_metrics["ai_offspring_added"],
			) = get_elites(current_metrics["is_offspring_added"])
			(
				current_metrics["ga_improvement"],
				current_metrics["qpg_improvement"],
				current_metrics["ai_improvement"],
			) = get_elites(current_metrics["improvement"])
		...
		log_metrics["qpg_offspring_added"] = jnp.sum(
			current_metrics["qpg_offspring_added"]
		)
		log_metrics["ai_offspring_added"] = jnp.sum(
			current_metrics["ai_offspring_added"]
		)
	...
	state_dict = serialization.to_state_dict(
		emitter_state.emitter_states[0].actor_params
	)
